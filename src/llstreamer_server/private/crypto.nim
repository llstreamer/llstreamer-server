...
var cryptoThread: Thread[ptr JobQueue]
proc cryptoThreadProc(queue: ptr JobQueue) {.thread.} =
...
proc initCryptoWorker*() =
    ## Initializes the crypto worker thread

    createThread(cryptoThread, cryptoThreadProc, addr jobQueue)
